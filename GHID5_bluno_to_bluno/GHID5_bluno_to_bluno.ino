...
//    } 
     ...
     //TODO: sometimes it reads a huge number!
       if(Serial.available())
        {
          senVal = Serial.parseInt(); //If accepts a DEC number in buffer, copy it otherwise is ZERO
          //TODO : look into this...
           if (senVal >= 0 && senVal <= 1023) { //If a DEC number was recieved, SenVal will equal 0 if its not an integer in the serial buffer!          
               if (senVal == 1023) {
